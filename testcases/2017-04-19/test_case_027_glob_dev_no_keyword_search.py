...
class TestCase027GlobDevNoKeywordSearch(unittest.TestCase):
    ...
    def test_global_dev_no_keyword_search(self):
        ...
        for row in user_relation:
            ...
            # 读取数据
            dev_search_data = cur.fetchall()
            total_list = []
            for range1 in dev_search_data:
                for range2 in range1:
                    total_list.append(range2)
            # 从数据tuple中获取最终查询记录统计条数
            dev_search_count = len(total_list)

            print("当前登录账户与其所有下级账户中搜有设备共：" + str(dev_search_count) + "!")
            ...
            # 获取搜索结果总数
            result_num = self.global_dev_search_page.easy_search_result()

            # 验证搜索结果与数据库查询结果是否一致
            self.assertEqual(dev_search_count, result_num, "不输入搜索条件时搜索的结果与数据库查询结果总数不一致")
