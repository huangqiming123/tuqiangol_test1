...
# 全局搜索-高级搜索-通过日期类型单一查找

# author:孙燕妮

class TestCase045GlobComplexSearchByDateType(unittest.TestCase):
    def setUp(self):
    ...
    def test_glob_complex_search_by_date_type(self):
        '''测试全局搜索-高级搜索-通过日期类型单一查找功能'''
        ...
        for row in user_relation:
            ...
            print("当前所选激活时间匹配的设备共：" + str(dev_count_01) + "条!")




            '''# 执行sql脚本，通过当前所选用户到期时间查找所有匹配记录
            get_result_sql_02 = "select e.imei from assets_device_expiration  e left join assets_device d  on e.imei = d.imei " \
                                "where e.userId in " + str(lower_account_tuple) + \
                                "and e.expiration > '2017-02-23' and e.expiration < '2017-11-02' group by d.imei;"
            ...
            print("当前所选平台到期时间匹配的设备共：" + str(dev_count_03) + "条!")
            ...
            # 编辑日期信息
            self.global_complex_search_page.complex_search_select_date_type("用户到期时间", "2017-02-23", "2017-11-02")
