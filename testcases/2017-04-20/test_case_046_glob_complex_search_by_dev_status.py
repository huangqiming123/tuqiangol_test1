...
# 全局搜索-高级搜索-通过设备状态单一查找

# author:孙燕妮

class TestCase046GlobComplexSearchByDevStatus(unittest.TestCase):
    def setUp(self):
    ...
    def test_glob_complex_search_by_dev_status(self):
        ...
        for row in user_relation:
            ...
            # 执行sql脚本，通过当前所选设备状态-欠费 查找所有匹配记录
            get_result_sql_01 = "select e.id from assets_device_expiration  e left join assets_device d  on e.imei = d.imei " \
                                "where e.userId in " + str(lower_account_tuple) + "  and CURDATE() > e.expiration group by d.imei;"
            ...
            # 从数据tuple中获取当前所选欠费状态的设备个数
            dev_count_01 = len(total_list)

            print("当前所选欠费状态的设备共：" + str(dev_count_01) + "条!")

            # 执行sql脚本，通过当前所选设备状态-未激活 查找所有匹配记录
            ...
            # 从数据tuple中获取当前所选未激活状态的设备个数
            dev_count_02 = len(total_list)

            print("当前所选未激活状态的设备共：" + str(dev_count_02) + "条!")
