...
# 全局搜索-高级搜索-通过日期类型+设备状态双组合查找

# author:孙燕妮

class TestCase052GlobComplexSearchByDateTypeAndDevStatus(unittest.TestCase):
    def setUp(self):
    ...
    def test_glob_complex_search_by_date_type_and_dev_status(self):
        '''测试全局搜索-高级搜索-通过日期类型+设备状态双组合查找功能'''
        ...
        for row in user_relation:
            ...
            # 执行sql脚本，通过当前所选激活时间+设备状态-欠费 查找所有匹配记录
            get_result_sql_01 = "select e.id from assets_device_expiration  e left join assets_device d  on e.imei = d.imei " \
                                "where e.userId in " + str(lower_account_tuple) + "  and CURDATE() > e.expiration and " \
                                                                                  "d.activationTime between '2016-02-08' and '2017-02-24' group by d.imei;"
            ...
            # 从数据tuple中获取当前所选激活时间+欠费状态的设备个数
            dev_count_01 = len(total_list)

            print("当前所选激活时间+欠费状态的设备共：" + str(dev_count_01) + "条!")
