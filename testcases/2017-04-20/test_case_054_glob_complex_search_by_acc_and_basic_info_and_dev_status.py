...
# 全局搜索-高级搜索-通过选择用户+基本信息+设备状态三组合查找

# author:孙燕妮

class TestCase054GlobComplexSearchByAccAndBasicInfoAndDevStatus(unittest.TestCase):
    def setUp(self):
    ...
    def test_glob_complex_search_by_acc_and_basic_info_and_dev_status(self):
        '''测试全局搜索-高级搜索-通过选择用户+基本信息+设备状态三组合查找功能'''
        ...
        # 执行sql脚本，通过当前所选用户+基本信息+设备状态-欠费 查找所有匹配记录
        get_result_sql_01 = "select e.id from assets_device_expiration  e left join assets_device d  on e.imei = d.imei " \
                            "where e.userId = '" + curr_userId + "'  and CURDATE() > e.expiration and d.imei like '%" \
        ...
        # 从数据tuple中获取当前所选用户+基本信息+欠费状态的设备个数
        dev_count_01 = len(total_list)

        print("当前所选用户+基本信息+欠费状态的设备共：" + str(dev_count_01) + "条!")

        # 执行sql脚本，通过当前所选用户+基本信息+设备状态-未激活 查找所有匹配记录
        get_result_sql_02 = "select id from assets_device where userId = '" + curr_userId + \
        ...
        # 从数据tuple中获取当前所选用户+基本信息+未激活状态的设备个数
        dev_count_02 = len(total_list)

        print("当前所选用户+基本信息+未激活状态的设备共：" + str(dev_count_02) + "条!")
