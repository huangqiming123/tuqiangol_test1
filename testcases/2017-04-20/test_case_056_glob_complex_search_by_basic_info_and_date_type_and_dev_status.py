...
# 全局搜索-高级搜索-通过基本信息+日期类型+设备状态三组合查找

# author:孙燕妮

class TestCase056GlobComplexSearchByBasicInfoAndDateTypeAndDevStatus(unittest.TestCase):
    def setUp(self):
    ...
    def test_glob_complex_search_by_basic_info_and_date_type_and_dev_status(self):
        '''测试全局搜索-高级搜索-通过基本信息+日期类型+设备状态三组合查找功能'''
        ...
        for row in user_relation:
            ...
            get_result_sql_01 = "select e.id from assets_device_expiration  e " \
                                "left join assets_device d  on e.imei = d.imei where d.userId in " + str(lower_account_tuple) +\
                                "  and CURDATE() > e.expiration and d.activationTime between '2016-02-08' and '2017-02-24'" \
