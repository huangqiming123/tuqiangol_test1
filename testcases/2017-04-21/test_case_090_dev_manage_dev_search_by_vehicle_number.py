...
class TestCase090DevManageDevSearchByVehicleNumber(unittest.TestCase):
    def setUp(self):
    ...
    def test_dev_manage_dev_search_by_vehicle_number(self):
        '''测试设备管理-设备搜索-by 车牌号'''
        ...
        for row in csv_data:
            ...
            # 执行sql脚本，通过当前登录账号userId查找该用户下车牌号等于或包含search_info["vehicle_number"]的所有设备记录
            get_login_user_dev_sql = "select id from assets_device where userId = '" + login_userId + \
                                     "' and vehicleNumber like '%" + search_info["vehicle_number"] + "%';"
            ...
            # 输入车牌号字符
            self.dev_manage_page.input_vehicle_number(search_info["vehicle_number"])
            ...
            # 执行sql脚本，通过当前选中的账号userId查找该用户下车牌号等于或包含search_info["vehicle_number"]的所有设备记录
            ...
            # 从数据tuple中获取当前选中的账号下符合搜索条件的设备个数
            curr_user_dev_count = len(total_list)

            print("当前选中的账户" + search_info["user_select"] + "符合搜索条件的设备共：" + str(curr_user_dev_count) + "条!")
            ...
            # 获取当前搜索结果设备个数
            curr_user_dev_num = self.dev_manage_page.count_curr_dev_num()

            # 验证搜索结果与数据库查询结果是否一致
            self.assertEqual(curr_user_dev_count, curr_user_dev_num, "搜索结果与数据库查询结果不一致")
