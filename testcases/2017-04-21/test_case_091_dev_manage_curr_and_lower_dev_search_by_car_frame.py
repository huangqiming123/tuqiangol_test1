...
# 设备管理-包含下级的设备搜索-by 车架号

# author:孙燕妮

class TestCase091DevManageCurrAndLowerDevSearchByCarFrame(unittest.TestCase):
    def setUp(self):
    ...
    def test_dev_manage_curr_and_lower_dev_search_by_car_frame(self):
        '''测试设备管理-包含下级的设备搜索-by 车架号'''
        ...
        for row in csv_data:
            ...
            for row in user_relation:
                ...
                # 执行sql脚本，通过当前登录账号及其下级账户的userID查找货架号等于或包含search_info["car_frame"]的所有设备记录
                get_login_user_dev_sql = "select id from assets_device where userId in " + str(lower_account_tuple) + \
                                         " and carFrame like '%" + search_info["car_frame"] + "%';"
                ...
                print("当前登录账户jimitest及其下级账户符合搜索条件的设备共：" + str(login_user_dev_count) + "条!")
