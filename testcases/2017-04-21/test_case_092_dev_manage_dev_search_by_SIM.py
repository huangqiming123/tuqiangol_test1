...
class TestCase092DevManageDevSearchBySIM(unittest.TestCase):
    def setUp(self):
    ...
    def test_dev_manage_dev_search_by_SIM(self):
        '''测试设备管理-设备搜索-by SIM'''
        ...
        for row in csv_data:
            ...
            # 执行sql脚本，通过当前登录账号userId查找该用户下sim卡号等于或包含search_info["SIM"]的所有设备记录
            get_login_user_dev_sql = "select id from assets_device where userId = '" + login_userId + \
                                     "' and sim like '%" + search_info["SIM"] + "%';"
            ...
            # 输入SIM卡号字符
            self.dev_manage_page.input_SIM(search_info["SIM"])
            ...
            # 执行sql脚本，通过当前选中的账号userId查找该用户下sim卡号等于或包含search_info["vehicle_number"]的所有设备记录
