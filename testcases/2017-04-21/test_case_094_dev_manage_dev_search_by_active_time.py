...
class TestCase094DevManageDevSearchByActiveTime(unittest.TestCase):
    def setUp(self):
    ...
    def test_dev_manage_dev_search_by_active_time(self):
        '''测试设备管理-设备搜索-by 激活时间'''
        ...
        for row in csv_data:
            ...
            # 执行sql脚本，通过当前登录账号userId查找该用户下设备激活时间在搜索时间段内的所有设备记录
            get_login_user_dev_sql = "select id from assets_device where userId = '" + login_userId + \
                                     "' and activationTime between '" + search_info["active_start_time"] + \
