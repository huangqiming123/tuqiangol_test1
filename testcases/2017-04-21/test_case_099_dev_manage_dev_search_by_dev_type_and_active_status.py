...
# 设备管理-包含下级的设备搜索-by 设备类型+激活状态（未激活）

# author:孙燕妮

class TestCase099DevManageDevSearchByDevTypeAndActiveStatus(unittest.TestCase):
    def setUp(self):
    ...
    def test_dev_manage_dev_search_by_dev_type_and_expired(self):
        '''测试设备管理-包含下级的设备搜索-by 设备类型+激活状态（未激活）'''
        ...
        for row in csv_data:
            ...
            for row in user_relation:
                ...
                # 执行sql脚本，通过当前登录账号及其下级用户查找该用户下机型名称为search_info["login_user_dev_type"]且未激活的所有设备记录
                get_login_user_dev_sql = "select a.id from assets_device a where a.userId in " + str(lower_account_tuple) + " and a.mcType = '" + \
                                         search_info["login_user_dev_type"] + "' and a.activationTime is NULL;"
                ...
                print("当前登录账户jimitest下机型名称为" + search_info["login_user_dev_type"] + "的未激活设备共：" + str(login_user_dev_count) + "条!")
