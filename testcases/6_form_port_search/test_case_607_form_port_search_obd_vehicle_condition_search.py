...
class TestCase607FormPortSearchObdVehicleConditionSearch(unittest.TestCase):
    # 测试 报表 接口搜索 obd车况报表
    def setUp(self):
        # 前置条件
        # 实例化对象
    ...
    def test_case_obd_vehicle_condition_port_search(self):
        # 断言url
        ...
        for row in csv_data:
            ...
            if total_page == 0:
                ...
                for data in res_data:
                    del data['acc'], data['direction'], data['engineLoad'], data['fuelConsumption2'], data[
                        'gpsInfo'], data['gpsSpeed'], data['heatingTime'], data['idleTime'], data['imei'], data['lat'], \
                        data['lng'], data['maxSpeed'], data[
                        'oilPer'], data['rapidAcceleration'], data['rapidDeceleration'], data['throttlePosition']
