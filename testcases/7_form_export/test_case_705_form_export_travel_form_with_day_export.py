...
class TestCase705FormExportTravelFormWithDayExport(unittest.TestCase):
    # 测试 报表 行程报表-按天
    def setUp(self):
        # 前置条件
        # 实例化对象
    ...
    def test_case_travel_form_with_day_export(self):
        # 断言url
        ...
        # 让其展示所有列
        # 点击展示列
        self.form_page.click_display_line_button_travel_with_day()
        # 获取有多少个展示列
        display_line_number = self.form_page.get_display_line_number_travel_with_day()
        for n in range(display_line_number):
            # 获取每一个展示列是否被勾选
            display_style = self.form_page.get_per_display_style_travel_with_day(n)
            if display_style == False:
                self.form_page.click_per_display_input_button_travel_with_day(n)
        self.form_page.click_display_line_button_travel_with_day()
        ...
        if total_page == 0:
            ...
            if total_page == 1:
                ...
                for a in range(total_number_per_page):
                    web_data.append(self.form_export_page.get_per_line_data_travel_with_day(a))
