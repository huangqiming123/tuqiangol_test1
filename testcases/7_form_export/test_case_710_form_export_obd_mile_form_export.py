...
class TestCase710FormExportOBDMileFormExport(unittest.TestCase):
    ...
    def test_case_obd_mile_form_export(self):
        # 断言url
        ...
        # 搜索数据
        self.form_export_page.search_obd_mileage_data()
        ...
        # 获取页面中的数据
        web_data = []
        total_page = self.statistical_form_page4.get_total_page_in_obd_mileage()
        if total_page == 0:
            pass
        else:
            if total_page == 1:
                total_number_per_page = self.statistical_form_page4.get_total_number_per_page_in_obd_mileage()
                for a in range(total_number_per_page):
                    web_data.append(self.form_export_page.get_per_line_data_obd_mileage(a))
