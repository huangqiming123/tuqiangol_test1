...
class TestCase711FormExportOBDTravelFormExport(unittest.TestCase):
    # 测试 obd报表 行程 - 导出
    def setUp(self):
        # 前置条件
        # 实例化对象
    ...
    def test_case_obd_travel_form_export(self):
        # 断言url
        ...
        # 搜索数据
        self.form_export_page.search_obd_travel_data()
        ...
        # 获取页面中的数据
        web_data = []
        total_page = self.statistical_form_page4.get_total_page_in_obd_travel()
        if total_page == 0:
            pass
        else:
            if total_page == 1:
                total_number_per_page = self.statistical_form_page4.get_total_number_per_page_in_obd_travel()
                for a in range(total_number_per_page):
                    web_data.append(self.form_export_page.get_per_line_data_obd_travel(a))
