...
class TestCase715FormExportClockFormExport(unittest.TestCase):
    # 测试 打卡报表 - 导出
    def setUp(self):
        # 前置条件
        # 实例化对象
    ...
    def test_case_clock_form_export(self):
        # 断言url
        ...
        # 切换到里程报表的frame
        self.statistical_form_page.click_clock_report_button()
        self.statistical_form_page.switch_to_clock_report_frame()
        # 搜索数据
        self.form_export_page.search_clock_data()

        # 让其展示所有列
        # 点击展示列
        self.form_page.click_display_line_button_guide()
        # 获取有多少个展示列
        display_line_number = self.form_page.get_display_line_name_number_clock()
        print(display_line_number)
        for n in range(display_line_number):
            # 获取每一个展示列是否被勾选
            display_style = self.form_page.get_per_display_style_clock(n)
            if display_style == False:
                self.form_page.click_per_display_input_button_clock(n)
        sleep(2)
        self.form_page.click_display_line_button_guide()
        # 获取页面中的数据
        web_data = []
        total_page = self.statistical_form_page4.get_total_page_in_clock()
        if total_page == 0:
            pass
        else:
            if total_page == 1:
                total_number_per_page = self.statistical_form_page4.get_total_number_per_page_in_clock()
                for a in range(total_number_per_page):
                    web_data.append(self.form_export_page.get_per_line_data_clock(a))
