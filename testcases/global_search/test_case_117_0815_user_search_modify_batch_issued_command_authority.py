...
class TestCase117UserSearchModifyBatchIssuedCommandAuthority(unittest.TestCase):
    def setUp(self):
    ...
    def test_case_user_search_modify_batch_issued_command_authority(self):
        # 打开途强在线首页-登录页
        ...
        if batch_issued_command_authority == True:
            ## 点击web修改设备的权限
            self.global_account_search_page.click_modify_batch_issued_command_authority_in_user_detail()
            # 点击保存
            ...
            select_issued_command = self.assert_text.select_issued_command_text()
            self.assertNotIn(select_issued_command, operation_all)
            all_issued_command = self.assert_text.all_issued_command_text()
            self.assertNotIn(all_issued_command, operation_all)
            ...
            select_issued_command = self.assert_text.select_issued_command_text()
            self.assertIn(select_issued_command, operation_all)
            all_issued_command = self.assert_text.all_issued_command_text()
            self.assertIn(all_issued_command, operation_all)

        elif batch_issued_command_authority == False:
            ## 点击web修改设备的权限
            self.global_account_search_page.click_modify_batch_issued_command_authority_in_user_detail()
            # 点击保存
            self.global_account_search_page.click_ensuer_button_in_user_detail()
