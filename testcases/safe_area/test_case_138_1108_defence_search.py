...
class TestCase138DefenceSearch(unittest.TestCase):
    def setUp(self):
    ...
    def test_case_138_defence_search(self):
        # 点击账号中心
        sleep(5)
        ...
        csv_file = self.safe_area_page_read_csv.read_csv('search_defence.csv')
        ...
        for row in csv_data:
            ...
            self.safe_area_page.add_data_to_search_defence_in_safe_area(data)
            # 获取查询出来的页面条数
            web_total = self.safe_area_page.get_web_total_after_click_search()
            sql_total = self.safe_area_page.get_sql_total_after_click_search(data, account)
