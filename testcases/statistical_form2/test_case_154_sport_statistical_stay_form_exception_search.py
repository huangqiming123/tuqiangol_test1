...
class TestCase154SportStatisticalStayFormExceptionSearch(unittest.TestCase):
    ...
    def test_stay_form_exception_search(self):
        ...
        # 填写下级的imei搜索
        sleep(2)
        self.statistical_form_page2.input_imei_to_search_in_stay_form(self.statistical_form_page2.get_imei())
        # 断言
        # 获取查询设备的imei
        ...
        # 验证停机的设备无法搜索到
        self.statistical_form_page2.input_imei_to_search_in_stay_form(self.statistical_form_page2.get_shut_down_imei())
        # 获取搜索的数量
        get_number_after_search = self.statistical_form_page.get_number_after_search_in_stay_form()
        self.assertEqual(0, get_number_after_search)

        get_text_after_search = self.statistical_form_page.get_text_after_search_in_stay_form()
        self.assertIn(self.assert_text.account_center_page_no_data_text(), get_text_after_search)

        # 验证未激活的设备无法搜索到
        self.statistical_form_page2.input_imei_to_search_in_stay_form(self.statistical_form_page2.get_no_active_imei())
        # 获取搜索的数量
        get_number_after_search = self.statistical_form_page.get_number_after_search_in_stay_form()
        self.assertEqual(0, get_number_after_search)
