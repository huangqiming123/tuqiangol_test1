...
class TestCase176SportStatisticalTracelReportForm(unittest.TestCase):
    '''
    运动统计 行程报表
    author:zhangAo
    '''
    ...
    def test_case_sport_statistical_tracel_report_form(self):
        ...
        for row in csv_data:
            ...
            if search_data['type'] == 'mile':
                ...
                if total_page == 0:
                    ...
                    for n in range(total_number_per_page):
                        web_data.append({
                            'imei': self.statistical_form_page3.get_imei_in_tracel_form(n),
                ...
                if total_page == 0:
                    ...
                    for n in range(total_number_per_page):
                        web_data.append({
                            'imei': self.statistical_form_page3.get_imei_in_tracel_form_with_day(n),
                            'distance': self.statistical_form_page3.get_distance_in_tracel_report_form_with_day(n),
                            'atDay': self.statistical_form_page3.get_at_day_time_in_tracel_report_form_with_day(n)
                        })
                    print(web_data)
                    # 连接数据库
                    all_dev = self.seasrch_sql.search_current_account_equipment(search_data['search_user'])
            ...
            if response_data['code'] == 0:
                ...
                self.assertAlmostEqual(float(data_totals), float(web_total))
