...
class TestCase191OverspeedVerifyTheDisplayLine(unittest.TestCase):
    def setUp(self):
        # 前置条件
        # 实例化对象
    ...
    def test_case_191_overspeed_verify_the_display_line(self):
        # 断言url
        ...
        # 点击展示列
        self.form_page.click_display_line_button_overspeed()
        # 获取有多少个展示列
        display_line_number = self.form_page.get_display_line_number_overspeed()
        self.form_page.click_display_line_button_overspeed()
        for n in range(display_line_number):
            # 获取每一个展示列是否被勾选
            self.form_page.click_display_line_button_overspeed()
            display_style = self.form_page.get_per_display_style_overspeed(n)
            display_line_name = self.form_page.get_per_display_name_overspeed(n)
            # 点击
            self.form_page.click_per_display_input_button_overspeed(n)
            # 点击展示列
            self.form_page.click_display_line_button_overspeed()
            # 获取展示列数量
            display_line_name_list = []
            display_line_name_number = self.form_page.get_display_line_name_number_overspeed()
            for m in range(display_line_name_number):
                display_name = self.form_page.get_per_display_name_on_line_overspeed(m)
