...
class TestCase192StayVerifyTheDisplayLine(unittest.TestCase):
    def setUp(self):
        # 前置条件
        # 实例化对象
    ...
    def test_case_192_stay_verify_the_display_line(self):
        # 断言url
        ...
        for n in range(display_line_number):
            ...
            display_line_name = self.form_page.get_per_display_name_stay(n)
            # 点击
            ...
            display_line_name_number = self.form_page.get_display_line_name_number_stay()
            for m in range(display_line_name_number):
                display_name = self.form_page.get_per_display_name_on_line_stay(m)
