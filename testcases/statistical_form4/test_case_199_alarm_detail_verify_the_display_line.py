...
class TestCase199AlarmDetailVerifyTheDisplayLine(unittest.TestCase):
    def setUp(self):
        # 前置条件
        # 实例化对象
    ...
    def test_case_199_alarm_detail_verify_the_display_line(self):
        # 断言url
        ...
        for n in range(display_line_number):
            ...
            for n1 in range(display_line_number):
                display_styles = self.form_page.get_per_display_style_alarm_detail(n1)
                if display_styles == True:
                    display_style_list.append(display_styles)
            display_line_name = self.form_page.get_per_display_name_alarm_detail(n)
            # 点击
            ...
            display_line_name_number = self.form_page.get_display_line_name_number_alarm_detail()
            for m in range(display_line_name_number):
                display_name = self.form_page.get_per_display_name_on_line_alarm_detail(m)
