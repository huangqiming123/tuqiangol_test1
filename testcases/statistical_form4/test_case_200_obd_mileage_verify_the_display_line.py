...
class TestCase200ObdMileageVerifyTheDisplayLine(unittest.TestCase):
    def setUp(self):
        # 前置条件
        # 实例化对象
    ...
    def test_case_200_obd_mileage_verify_the_display_line(self):
        # 断言url
        ...
        self.statistical_form_page.click_obd_mileage_reoport()
        self.statistical_form_page.switch_to_obd_mileage_frame()
        # 点击展示列
        self.form_page.click_display_line_button_obd_mileage()
        # 获取有多少个展示列
        display_line_number = self.form_page.get_display_line_number_obd_mileage()
        self.form_page.click_display_line_button_obd_mileage()
        for n in range(display_line_number):
            # 获取每一个展示列是否被勾选
            self.form_page.click_display_line_button_obd_mileage()
            display_style = self.form_page.get_per_display_style_obd_mileage(n)
            display_style_list = []
            for n1 in range(display_line_number):
                display_styles = self.form_page.get_per_display_style_obd_mileage(n1)
                if display_styles == True:
                    display_style_list.append(display_styles)
            display_line_name = self.form_page.get_per_display_name_obd_mileage(n)
            # 点击
            self.form_page.click_per_display_input_button_obd_mileage(n)
            # 点击展示列
            self.form_page.click_display_line_button_obd_mileage()
            # 获取展示列数量
            display_line_name_list = []
            display_line_name_number = self.form_page.get_display_line_name_number_obd_mileage()
            for m in range(display_line_name_number):
                display_name = self.form_page.get_per_display_name_on_line_obd_mileage(m)
