...
class TestCase203FailureTravelVerifyTheDisplayLine(unittest.TestCase):
    def setUp(self):
        # 前置条件
        # 实例化对象
    ...
    def test_case_203_failure_verify_the_display_line(self):
        # 断言url
        ...
        self.statistical_form_page.click_obd_failure_reoport()
        self.statistical_form_page.switch_to_obd_failure_frame()
        # 点击展示列
        self.form_page.click_display_line_button_obd_failure()
        # 获取有多少个展示列
        display_line_number = self.form_page.get_display_line_number_obd_failure()
        self.form_page.click_display_line_button_obd_failure()
        for n in range(display_line_number):
            ...
            for n1 in range(display_line_number):
                display_styles = self.form_page.get_per_display_style_obd_failure(n1)
                if display_styles == True:
                    display_style_list.append(display_styles)
            display_line_name = self.form_page.get_per_display_name_obd_failure(n)
            # 点击
            ...
            display_line_name_number = self.form_page.get_display_line_name_number_obd_failure()
            for m in range(display_line_name_number):
                display_name = self.form_page.get_per_display_name_on_line_obd_failure(m)
