...
class TestCase211StayNotShutDownExport(unittest.TestCase):
    # 停车未熄火报表 - 导出
    def setUp(self):
        # 前置条件
        # 实例化对象
    ...
    def test_case_211_stay_not_shut_down_export(self):
        # 断言url
        ...
        self.statistical_form_page.switch_to_stay_not_shutdown_report_form_frame()
        # 搜索数据
        self.form_export_page.search_stay_not_shut_down_data()

        # 让其展示所有列
        # 点击展示列
        self.form_page.click_display_line_button_stay_not_shutdown()
        # 获取有多少个展示列
        display_line_number = self.form_page.get_display_line_number_stay_not_shutdown()
        for n in range(display_line_number):
            # 获取每一个展示列是否被勾选
            display_style = self.form_page.get_per_display_style_stay_not_shutdowm(n)
            if display_style == False:
                self.form_page.click_per_display_input_button_stay_not_shutdown(n)
        self.form_page.click_display_line_button_stay_not_shutdown()
        # 获取页面中的数据
        web_data = []
        total_page = self.statistical_form_page4.get_total_page_in_over_stay_not_shut_down_form()
        if total_page == 0:
            pass
        else:
            if total_page == 1:
                total_number_per_page = self.statistical_form_page4.get_total_number_per_page_in_over_stay_not_shut_down_form()
                for a in range(total_number_per_page):
                    web_data.append(self.form_export_page.get_per_line_data_stay_not_shut_down(a))
